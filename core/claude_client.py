...
• AI가 생성하거나 확인되지 않은 추정값은 반드시 "⚠️ AI 추정값 — 제출 전 직접 확인 필요" 로 표시하라.
• 출처 불명 정보는 (출처 불명 — 검증 필요) 로 명시하거나 해당 항목을 삭제하라.
• '자체 분석', '자체 추정', '~로 알려진' 등 모호한 표현 단독 사용 금지.
• 위 규칙을 위반한 수치·통계는 검수 단계에서 모두 제거된다.

[데이터 적합성 원칙 — 절대 준수]
• 인용 데이터는 반드시 현재 다루는 주제와 직접 관련된 것이어야 한다.
• 유사하지만 다른 주제의 통계·사례 사용 금지:
  - 데이트 폭력 주제 → 가정 폭력 통계 인용 (X)
  - 청소년 도박 주제 → 성인 도박 통계 인용 (X)
  - 온라인 범죄 주제 → 오프라인 범죄 통계 인용 (X)
• 주제와 100% 일치하는 데이터만 사용. 일치 데이터 없으면 "관련 데이터 없음"으로 표기.
• 절대 유사 데이터로 대체하지 말 것. 주제 불일치 데이터는 삭제하라."""
