...
def _build_pt_qa_prompt(
    ...
    pt_min   = max(3, min(30, getattr(dna, "pt_duration_min", 10) or 10))
    pt_chars = pt_min * 250
    open_min  = max(1, round(pt_min * 0.15))
    close_min = max(1, round(pt_min * 0.20))
    body_min  = pt_min - open_min - close_min
...
━━━━━━━━━━━━━━━━━━━━━━━
발표 시간: {pt_min}분 기준
목표 분량: {pt_chars}자 이상 (한국어 분당 약 250자)

절대 요약하지 말고 실제 읽었을 때 {pt_min}분이 되는 분량으로 작성.
구어체로 자연스럽게 작성 ("~습니다", "~드리겠습니다").
...
② 본론 key_points 5~7개 (총 {body_min}분)
   ...
   - 오프닝/클로징 포함 실제 발표 시 {pt_min}분 분량이 되도록 충분히 작성.
