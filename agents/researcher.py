...
8. AI 추정값 기재 절대 금지:
   - 확인되지 않은 수치는 아예 기재하지 말 것 (⚠️ 표시·주석 포함 금지)
   - "~로 알려져 있다", "~로 추정된다" 형태의 서술 금지
   - 대신 "해당 수치는 [기관명] [자료명]에서 직접 확인 필요" 형태로만 안내
9. 출처 불명 항목 기재 금지:
   - "(출처 불명 — 검증 필요)" 형태로 기재하는 것 자체를 금지
   - 출처가 없으면 해당 내용 전체를 삭제하고 기재하지 말 것
10. Perplexity 검색 결과 수치만 인용:
    - 검색으로 확인된 수치만 (출처: 기관명, 연도) 형식으로 인용
    - 검색 결과에 없는 수치는 절대 생성하지 말 것
11. 공식 통계 우선순위:
    - 방송통신위원회, KISDI, 한국언론진흥재단 등 공신력 있는 기관 자료 우선
    - 민간 분석 리포트(캐치 등)는 "(비공식 출처 — 직접 확인 필요)" 표시

위 원칙 위반 시 해당 내용 전체를 삭제하고 기재하지 마십시오.
========================================
"""
...
"""
    ...
    def _call_one(key: str, label: str, search_keys: list, instructions: str) -> tuple[str, str]:
        ...
        prompt = (
            ...
            f"6. AI가 생성하거나 확인되지 않은 추정값은 기재 금지 — 대신 '해당 수치는 [기관명] [자료명]에서 직접 확인 필요' 형태로만 안내.\n"
            f"7. 【데이터 적합성】 인용 데이터는 현재 RFP 주제와 직접 관련된 것만 사용.\n"
            f"   유사하지만 다른 주제의 통계 사용 금지. 일치 데이터 없으면 '관련 데이터 없음' 표기.\n\n"
            f"[프로젝트 정보]\n{dna_ctx}\n\n"
            f"[DB 유사 케이스]\n{past_str}\n"
            f"{learning_block}\n\n"
            f"[검색 결과]\n{search_block}\n\n"
